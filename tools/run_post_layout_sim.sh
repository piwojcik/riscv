...
function usage {
    ...
    echo "      -n              disable timing checks (setup, hold, etc.)"
    echo "      -s,             specify a seed for simulation (generated automatically if not specified)"
    echo "      -t              dump TCF file from the simulation (time consuming!)"
    echo "      -y [min|max]    use min or max delays (default: max)."
    exit 1
}
...
function prepare_post_layout_data {
    . scripts/prepare_post_data.sh
    mkdir -p RESULTS
}
...
function execute_test {
    ...
    # TSMC params
    xrun_args+="-define NTC "
    xrun_args+="-define RECREM "
    xrun_args+="-define TSMC_INITIALIZE_MEM "
    xrun_args+="-exclude_file xminitialize.exclude "
    xrun_args+="-xmhierarchy \"tb_mtm_riscv_soc\" "
    xrun_args+="-v /cad/dk/PDK_CRN45GS_DGO_11_25/digital/Front_End/verilog/tcbn40lpbwp_200a/tcbn40lpbwp.v "
    ...
    if [[ ${delay} == "min" ]]; then
        xrun_args+="-mindelays "
        xrun_args+="-v /cad/dk/PDK_CRN45GS_DGO_11_25/IMEC_RAM_generator/ram/ts1n40lpb4096x32m4m_250a/VERILOG/ts1n40lpb4096x32m4m_250a_ff1p21v0c.v "
    else
        xrun_args+="-maxdelays "
        xrun_args+="-v /cad/dk/PDK_CRN45GS_DGO_11_25/IMEC_RAM_generator/ram/ts1n40lpb4096x32m4m_250a/VERILOG/ts1n40lpb4096x32m4m_250a_ss0p99v125c.v "
    fi

    if [[ ${tcf_dump} ]]; then
        xrun_args+="-access r "
        xrun_args+="-tcl -input scripts/dumptcf.tcl "
    fi

    if [[ ${no_timing_checks} ]]; then
        xrun_args+="-notimingchecks "
    fi

    # Simulation config
    xrun_args+="-64bit "
    xrun_args+="-define KMIE_IMPLEMENT_ASIC "
    xrun_args+="-define KMIE_POST_LAYOUT_SIM "
    xrun_args+="-F post_layout.f "
...
while getopts dgns:ty: option; do
    case ${option} in
        d) debug=1;;
        g) gui=1;;
        n) no_timing_checks=1;;
        s) seed=${OPTARG};;
        t) tcf_dump=1;;
        y) delay=${OPTARG};;
        *) usage;;
...
if [[ ${delay} ]]; then
    if [[ ! ${delay} =~ ^(min|max)$ ]]; then
        echo "ERROR: incorrect delay"
        exit 1
    fi

    echo "INFO: used delay: ${delay}"
else
    delay=max
    echo "INFO: used default delay: ${delay}"
fi
...
cd $(dirname $(readlink -e "$0"))/../hw/sim/post_layout

compile_program
generate_boot_mem
generate_mem_init_file
prepare_post_layout_data
execute_test
verify_test_results

if [[ ${no_timing_checks} ]]; then
    echo "INFO: timing checks were disabled during simulation" | tee -a $LOG_FILE
fi

if [[ ${tcf_dump} ]]; then
    echo "INFO: TCF dump was enabled during simulation" | tee -a $LOG_FILE
fi
